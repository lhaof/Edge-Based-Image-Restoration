...
if isnan(Par(3)) || Par(1)<x_lbound || Par(1)>x_ubound || Par(2)<y_lbound || Par(2)>y_ubound
    Par(1) = max(x_lbound, min(x_ubound, Par(1)));
    Par(2) = max(y_lbound, min(y_ubound, Par(2)));
    Par(3) = mean(sqrt(sum((XY-repmat(Par(1:2),[n,1])).^2,2)));
end
end    %    TaubinNTN
