...
def imrestore_rgb(imname):
	...
	img_up = img[:boxminh,:,:]
	imgdisp = Image.fromarray(img_up,'RGB')
	display(imgdisp)
	...
	imgdisp = Image.fromarray(img_down,'RGB')
	display(imgdisp)
	...
	display(imgdisp)
	if isfile(imname_down):
		os.remove(imname_down)
	...
	imgdisp = Image.fromarray(img_left,'RGB')
	display(imgdisp)
	...
	display(imgdisp)
	if isfile(imname_left):
		os.remove(imname_left)
	...
	imgdisp = Image.fromarray(img_right,'RGB')
	display(imgdisp)
	...
	display(imgdisp)
	if isfile(imname_right):
		os.remove(imname_right)
	...
	bandsz = 20
	band = np.ndarray(shape=(bandsz,bandsz*len(intensity),3),dtype=np.uint8)
	for i in range(len(intensity)):
		band[:,i*bandsz:(i+1)*bandsz,:] = intensity[i]
		band[:,i*bandsz,:] = 0
	imgdisp = Image.fromarray(band,'RGB')
	display(imgdisp)
	...
	gradient_up = sobel(rgb2gray(img_up))
	gradient_down = sobel(rgb2gray(img_down))
	gradient_left = sobel(rgb2gray(img_left))
	gradient_right = sobel(rgb2gray(img_right))
	gradmag = []
	for i in range(len(edgelist)):
		h_ind = edgelist[i][0]
		w_ind = edgelist[i][1]
	...
	visu_grad = np.zeros(shape=(bandsz*2,bandsz*(len(intensity)+1),3),dtype=np.uint8)
	...
	imgdisp = Image.fromarray(visu_grad,'RGB')
	display(imgdisp)
			...
			for g in config:
				for cp in g:
					...
					if beta_c==1:
						...
						up_term = up_term + float(np.mean(np.square((lamda_u_c-lamda_v_t)/255.0)))
						down_term = down_term + 1
					if beta_t==1:
						lamda_u_t = intensity[(u+nedge-1)%nedge]
						lamda_v_c = intensity[v]
						up_term = up_term + float(np.mean(np.square((lamda_u_t-lamda_v_c)/255.0)))
						down_term = down_term + 1
	...
	for i in range(lumin_diff.shape[0]):
		e = sparse_edge[i]
		lamda_e_c = intensity[e]
		lamda_e_t = intensity[(e+nedge-1)%nedge]
		lumin_diff[i] = 0-math.sqrt(np.mean(np.square(lamda_e_c-lamda_e_t)))
	...
	for i in range(len(sorted_groups)):
		for cp in sorted_groups[i]:
			...
			if pixel_arr.size==0: continue
			print 'pixel_arr.shape: ',pixel_arr.shape
			print np.min(pixel_arr), np.max(pixel_arr)
			ind = np.where(pixel_arr[:,0]<=boxmaxh)
			pixel_arr = np.squeeze(pixel_arr[ind,:],axis=0)
			if pixel_arr.size==0: continue
			print 'pixel_arr.shape: ',pixel_arr.shape
			print np.min(pixel_arr), np.max(pixel_arr)
			ind = np.where(pixel_arr[:,0]>=boxminh)
			pixel_arr = np.squeeze(pixel_arr[ind,:],axis=0)
			if pixel_arr.size==0: continue
			print 'pixel_arr.shape: ',pixel_arr.shape
			print np.min(pixel_arr), np.max(pixel_arr)
			ind = np.where(pixel_arr[:,1]<=boxmaxw)
			pixel_arr = np.squeeze(pixel_arr[ind,:],axis=0)
			if pixel_arr.size==0: continue
			print 'pixel_arr.shape: ',pixel_arr.shape
			print np.min(pixel_arr), np.max(pixel_arr)
			ind = np.where(pixel_arr[:,1]>=boxminw)
			pixel_arr = np.squeeze(pixel_arr[ind,:],axis=0)
			if pixel_arr.size==0: continue
			print 'pixel_arr.shape: ',pixel_arr.shape
			print np.min(pixel_arr), np.max(pixel_arr)
			pixel_arr_ = np.tile(np.expand_dims(pixel_arr,axis=1),(1,ofshw.shape[0],1))
			print 'pixel_arr_.shape: ',pixel_arr_.shape
			print np.min(pixel_arr_), np.max(pixel_arr_)
			ofshw_ = np.tile(np.expand_dims(ofshw,axis=0),(pixel_arr.shape[0],1,1))
			print 'ofshw_.shape: ',ofshw_.shape
			print np.min(ofshw_), np.max(ofshw_)
	...
	for i in range(1,np.max(connected_strip)+1):
		...
		for j in range(len(touch_bounds)):
			...
			if d2start_curve_id==-1 or d2end_curve_id==-1:
				continue
		...
		if len(P_list)==1:
			...
			P1 = P[0]
			P2 = P[1]
			lamda_P1 = img[P1[:,0].astype(np.int32),P1[:,1].astype(np.int32)]
			lamda_P2 = img[P2[:,0].astype(np.int32),P2[:,1].astype(np.int32)]
			Omega1 = np.tile(Omega[0].reshape(-1,1),(1,3))
			Omega2 = np.tile(Omega[1].reshape(-1,1),(1,3))
			lamda_P = (Omega1*lamda_P1+Omega2*lamda_P2)/(Omega1+Omega2)
			result[h_ind,w_ind] = lamda_P
			fill_flag[h_ind,w_ind] = True
		elif len(P_list)>1:
			sum_weight = np.zeros(shape=(strip_hw.shape[0],1),dtype=np.float32)
			sum_lamda = np.zeros(shape=(strip_hw.shape[0],3),dtype=np.float32)
			for j in range(len(touch_bounds)):
				P = P_list[j]
				Omega = Omega_list[j]
				P1 = P[0]
			...
			sum_lamda = sum_lamda/np.tile(sum_weight.reshape(-1,1),(1,3))
			result[h_ind,w_ind] = sum_lamda
			fill_flag[h_ind,w_ind] = True
	imgdisp = Image.fromarray(result,'RGB')
	display(imgdisp)

	h_ind,w_ind = np.where(fill_flag==False)
	print 'unfilled pixels num: %d'%(h_ind.size)

	return result

if __name__=='__main__':
	onlyfiles = []
	inputpath = 'paris_eval_gt_128'
	for f in listdir(inputpath):
		if isfile(join(inputpath,f)) and f.endswith('.png'):
			onlyfiles.append(f)
	print onlyfiles
	for i in range(len(onlyfiles)):
		imname = onlyfiles[i]
		print 'begin recovering image %d'%(i)
		start_time = time.time()
		fullname = join(inputpath,imname)
		result = imrestore_rgb(fullname)
		resname = join('output_128',imname)
		imsave(resname,result)
		print 'finish recovering image %d'%(i)
		print 'time cost: %.3f secs'%(time.time()-start_time)